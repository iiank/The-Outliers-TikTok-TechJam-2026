...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="title_cat",
        help="Text representation variant",
    )
    args = parser.parse_args()

    project_root = Path(__file__).resolve().parent.parent
    catalog_path = project_root / args.catalog
    persist_dir = project_root / args.persist_dir

    print(f"Building store from {catalog_path} into {persist_dir}...")
    store = build_store(
        catalog_path=catalog_path,
        persist_directory=persist_dir,
        variant=args.variant,
    )
    print(f"Successfully created Chroma store with {len(store)} vectors.")